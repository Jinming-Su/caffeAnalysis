// 参考: http://blog.csdn.net/shuzfan/article/details/52729424
...
void BatchNormLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // 分配是3个blobs, 分别记录均值滑动和、方差滑动和、滑动系数和
  // 具体计算方法参考: http://blog.csdn.net/shuzfan/article/details/52729424
  // 大小分别是 channels, channels, 1
...
void BatchNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  //如果底层的blob与顶层的blob不是同一个blob,进行copy
  if (bottom[0] != top[0]) {
  ...
  if (use_global_stats_) {
    ...
    void caffe_cpu_scale<float>(const int n, const float alpha, const float *x,
                                float* y) {
      cblas_scopy(n, x, 1, y, 1);  // copy, 向量的元素数量，向量x，使用x元素的间隔，向量y，使用y元素的间隔，
    ...
    */
    ...
    void caffe_cpu_gemv<float>(const CBLAS_TRANSPOSE TransA, const int M,
      ...
      Vector X.
      Stride within X. For example, if inc​X is 7, every 7th element is used.
    ...
    */
    caffe_cpu_gemv<Dtype>(CblasNoTrans, channels_ * num, spatial_dim,
        ...
        spatial_sum_multiplier_.cpu_data(), 0.,   //spatial_sum_multiplier之前初始化的是1
